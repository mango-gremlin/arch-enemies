...
func _physics_process(delta):
	...
	if not is_on_floor():
		...
		if velocity.y > -jump_velocity+10:
	...
	for i in get_slide_collision_count():
		...
		var map : BridgeMap = collision.get_collider()
		if map is BridgeMap:
			var tile_type = map.get_tile_type(0, map.local_to_map(global_position - collision.get_normal() * 16)) #0 is the current terrain!
			if tile_type == TileType.Variants.WATER or tile_type == TileType.Variants.FORBIDDEN:
				reset()
